...
//
// Third (final) onboarding screen: Help & FAQ (mirrors the Widgets HelpView —
// a scrollable list of section headers and collapsible question/answer rows,
// plus external-link buttons). It is the last step of the flow: Back returns to
// the Setup guide; Done ends onboarding (the host pops the flow and marks it
// complete). Presentation only.

// ComponentBehavior: Bound — the ListView delegate's inline Components reference
...
Item {
    ...
    signal back()  // -> previous screen (Setup guide)
    signal done()  // -> end onboarding
    ...
    ColumnLayout {
        ...
        Kit.SectionHeader { label: qsTr("Help & FAQ") }
        ...
        Kit.Card {
            ...
            contentItem: ListView {
                ...
                delegate: Item {
                    id: row
                    width: faqList.width
                    // Loader.implicitHeight tracks its loaded Item's implicit
                    // size, so the row sizes to its content.
                        ...
                        ColumnLayout {
                            ...
                            Button {
                                ...
                                contentItem: RowLayout {
                                    spacing: 6
                                    Label {
                                        text: header.checked ? "▼" : "▶" // down / right triangle
                                        color: Theme.muted
                                        font.pixelSize: 10
                                    }
                                    Label {
                                ...
                                background: Rectangle {
                                    color: header.hovered ? Qt.rgba(Theme.primary.r, Theme.primary.g, Theme.primary.b, 0.08)
                                                          : "transparent"
                                    radius: 6
                                }
                            }
                        ...
                        Kit.OutlineButton {
                            ...
                            onClicked: {
                                if (String(row.url) === "privacy")
                                    Qt.openUrlExternally(screen.privacyUrl)
                                else if (String(row.url) === "github")
                                    Qt.openUrlExternally(screen.githubUrl)
                            }
                        }
                    }
                }
