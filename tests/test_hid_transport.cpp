...
//
// HidTransport — the pure Bluetooth-vs-USB classification of a Windows HID
// device-interface path. Pins the marker set both consumers rely on: the
// WinHidGateway enumerate/claim filter (a BT pad must never become a USB-direct
// claim candidate) and the SDLGamepadBridge transport stamp (Device::bluetooth
// → the slot card's Bluetooth presentation). Paths are real-world shapes: the
// USB form embeds `vid_XXXX&pid_XXXX`, the Bluetooth-classic form the BTHENUM
// HID service UUID `{00001124-…}`, and BLE the BTHLEDevice enumerator.
...
TEST_CASE("HidTransport: the Bluetooth-classic HID service UUID reads as Bluetooth",
    ...
    // instead of the vid_/pid_ USB form. This is the exact shape whose leak
    // into enumerate() grew a bogus "USB PATH" control on a wireless pad.
...
TEST_CASE("HidTransport: BTHENUM / BTHLEDevice instance ids read as Bluetooth (any case)",
          "[hidtransport]") {
    // Device instance ids (not interface paths) also classify — the helper
    // accepts either, and matching is case-insensitive like Windows itself.
...
TEST_CASE("HidTransport: empty / unknown paths fail safe to not-Bluetooth", "[hidtransport]") {
    // Unknown -> false keeps today's wired presentation (no false BT badges,
    // and a claimable USB pad is never wrongly filtered).
    REQUIRE_FALSE(isBluetoothHidDevicePath(""));
