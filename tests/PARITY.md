...
| [x] | core/model/ModelsTest.kt | 9 | PURE | test_models.cpp, test_catalog_dto.cpp | covered | DTO defaults + `SessionResponse` full parse (per-controller result→code, replugFailed-keeps-type, terminal 401 NOT_PAIRED/BAD_PROOF) + `CatalogDto` forward-compat (unknown slug/feature survive). |
