# Test-parity matrix: dish-windows against dish-android

> **What this is.** A coverage map between the dish-android test suite and this
> repo's Catch2 suite. dish-windows re-implements behavior dish-android had
> already pinned, so the bar is that every android test covering a ported
> behavior has a counterpart here, plus the Windows-only concerns android never
> had. One row per android test **file**, grouped by subsystem. `~#` is that
...
Plus Windows-authored stores with no android `@Test`, pinning the android
...
1. **`isLow` / battery-from-wire trapped in `ui/SlotCard.cpp`.**
   ...
   **no pure seam**, and it **diverges** from android's `MainUiState.isLow`
   ...
   The fix is to lift the rule into `core/reducer/BatteryUi.h`, point `SlotCard`
